    ...
    <article className="group/post space-y-3 rounded-2xl bg-card p-5 shadow-sm">
      <div className="flex justify-between gap-3">
        ...
        {post.userId === user.id && (
          <PostMoreButton
            post={post}
            className="border-transparent opacity-0 transition-opacity duration-200 ease-in-out group-hover/post:opacity-100"
          />
        )}
      </div>
...
//video and image preview component
  ...
  return (
    ...
    >
      {attachments.map((item) => (
        <MediaPreview media={item} key={item.id} />
      ))}
    </div>
  );
}
